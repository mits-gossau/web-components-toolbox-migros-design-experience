...
const isHTMLMdxFlyoutItemElement = (element) => {
...
const Select = class {
  ...
  render() {
    ...
    return (h(Host, { onItemClick: ({ detail: { item } }) => {
        this.select(item);
      } }, h("mdx-select-button", { disabled: this.disabled, required: this.required, label: this.label, placeholder: !itemLabel, open: this.open, text: itemLabel || this.text }, h("div", { class: "select__information-wrapper" }, h("div", null, this.hint && (typeof this.error === 'undefined' || this.error === '') && (h("span", { class: "select__hint" }, this.hint)), this.error && h("mdx-error-hint", { text: this.error })), h("div", { class: "select__flyout", "aria-haspopup": "true", "aria-expanded": this.open ? 'true' : 'false' }, h("slot", null))))));
  }
