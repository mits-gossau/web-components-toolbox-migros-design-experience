import { r as registerInstance, h, H as Host, g as getElement } from './index-7e8279cb.js';
import { B as BUTTON_VARIANTS, a as BUTTON_SIZES, b as BUTTON_TYPE } from './exports-a4c2b43e.js';
...
const MdxDatePickerStyle0 = datePickerCss;
...
const DatePicker = class {
    ...
    getDates() {
        ...
        return chunkedDates.map((chunk) => (h("tr", { key: chunk.join('-'), class: "date-picker__table-row-dates" }, chunk.map((date) => {
            if (date === null) {
    ...
    getMonths() {
        return this.chunkArray(this.generateMonthsAndYears(this.selectedYearView), 3).map((chunk) => (h("tr", { key: chunk.join('-'), class: "date-picker__table-row-month" }, chunk.map((month) => (h("mdx-date-picker-tile", { key: month.shortMonthName, variant: TILE_VARIANTS.MONTH, slot: "tile", content: month.shortMonthName, subContent: month.year.toString(), tileValue: month.monthNumber, today: month.monthNumber === new Date().getMonth() && month.year === new Date().getFullYear(), selected: month.monthNumber === this.value.getMonth() && month.year === this.value.getFullYear() }))))));
    }
    getYears() {
        return this.chunkArray(this.years, 4).map((chunk) => (h("tr", { key: chunk.join('-'), class: "date-picker__table-row-year" }, chunk.map((year) => (h("mdx-date-picker-tile", { key: year, variant: TILE_VARIANTS.YEAR, slot: "tile", content: year.toString(), tileValue: year, today: year === new Date().getFullYear(), selected: year === this.value.getFullYear() }))))));
    }
    render() {
        return (h(Host, { key: '842215945e6d1bb3334b0837da6021836f1c689e' }, h("mdx-input", { key: 'bc5fb61721197dc20008e31dd7f9aab684ea2857', label: this.inputLabel, type: INPUT_TYPES.DATE, value: this.formatDateForInput(this.value), onClick: () => (this.open = !this.open) }, h("button", { key: '4e4bc661a3ecd65a83dd30d911601dfdb7197029', onClick: () => (this.open = !this.open), slot: "icon", class: "date-picker__input-icon-button", "aria-hidden": true, tabindex: -1 }, h("mdx-icon-calendar", { key: 'bae1f07c69a45a402b0eb42e240bda4362a1d6f4', slot: "icon", size: ICON_SIZES.SM }))), h("div", { key: 'cb2ce25f954a1d34a02f7305080df398c4134481', class: `date-picker${this.open === undefined ? ' date-picker--is-hidden' : this.open ? ' date-picker--animate-in' : ' date-picker--animate-out'}`, "aria-hidden": true }, this.selectedMode === 'date' ? (h("mdx-date-picker-header", { arrowUpAriaLabel: this.arrowUpAriaLabel, arrowDownAriaLabel: this.arrowDownAriaLabel }, h("button", { slot: "title", "data-testid": "select-month", onClick: () => (this.selectedMode = 'month'), class: "date-picker__header-button" }, new Date(this.selectedYearView, this.selectedMonthView, 1).toLocaleString(this.language, {
            month: 'long',
        }), h("mdx-icon-chevron-down", { size: ICON_SIZES.XS, class: "date-picker__header-arrow-icon" })), h("button", { slot: "title", "data-testid": "select-year", onClick: () => {
                this.selectedMode = 'year';
            }, class: "date-picker__header-button" }, this.selectedYearView, h("mdx-icon-chevron-down", { size: ICON_SIZES.XS, class: "date-picker__header-arrow-icon" })))) : (h("mdx-date-picker-header", null, h("span", { slot: "title", class: "date-picker__header-title" }, this.selectedMode === 'month' ? this.monthModeTitle : this.yearModeTitle))), this.selectedMode === 'date' ? (h("table", { class: "date-picker__table" }, h("thead", { class: "date-picker__table-heading" }, weekdaysByLanguage[this.language].map((weekday) => (h("mdx-date-picker-tile", { variant: TILE_VARIANTS.WEEKDAY, slot: "tile", content: weekday })))), h("tbody", { class: "date-picker__table-body" }, this.getDates()))) : this.selectedMode === 'month' ? (h("table", { class: "date-picker__table" }, h("tbody", { class: "date-picker__table-body-month" }, this.getMonths()))) : (h("table", { class: "date-picker__table" }, h("tbody", { class: "date-picker__table-body-year" }, this.getYears()))), this.selectedMode === 'date' ? (h("mdx-date-picker-footer", null, h("mdx-button-group", { slot: "footer" }, h("mdx-button", { variant: BUTTON_VARIANTS.PRIMARY, size: BUTTON_SIZES.SM, onClick: () => (this.open = false), type: BUTTON_TYPE.SUBMIT }, this.submitButtonLabel), h("mdx-button", { variant: BUTTON_VARIANTS.TERTIARY, size: BUTTON_SIZES.SM, onClick: () => (this.open = false), type: BUTTON_TYPE.RESET }, this.cancelButtonLabel)))) : (h("mdx-date-picker-footer", null, h("mdx-button", { slot: "footer", variant: BUTTON_VARIANTS.TERTIARY, size: BUTTON_SIZES.SM, onClick: () => (this.selectedMode = 'date') }, h("mdx-icon-arrow-left", { size: ICON_SIZES.SM }), h("span", null, this.returnButtonLabel)))))));
    }
    get host() { return getElement(this); }
};
DatePicker.style = MdxDatePickerStyle0;
